...
form.addEventListener( 'submit', function ( e ) {
		...
		bEmptyStartDate && bEmptyEndDate && bEmptyStartTime && bEmptyEndTime )
	{
		...
		formData.append( 'id-descuento', idDescuento )
		...
		const peticionProductos = $.ajax( {
			async      : true,
			crossDomain: true,
			// url        : 'http://ec2-18-231-153-185.sa-east-1.compute.amazonaws.com:8000/api/descuento/editar',
			url        : 'http://127.0.0.1:8000/api/descuento/editar',
			method     : 'POST',
			data       : formData,
			processData: false,
			contentType: false
