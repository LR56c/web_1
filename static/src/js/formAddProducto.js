	...
	.ready( function () {
		...
		peticionOfertas.done( function ( response ) {
			...
			ofertasResponse.forEach( function ( item ) {
				...
				$(`#${id}-check`).change( function ( e ) {
					if ( id !== lastSelectedOferta ) {
						$( `#${ lastSelectedOferta }-check` ).prop( 'checked', false )
						...
						$( `#selected-name` ).text(`${item['name']}`)
						$( `#selected-porc` ).text(`${item['porcentaje']}`)
						$( `#selected-fi` ).text(`${item['fecha_inicio']}`)
						$( `#selected-fe` ).text(`${item['fecha_fin']}`)
					}
					else {
						$( `#${ lastSelectedOferta }-check` ).prop( 'checked', false )
		...
		form.submit( function ( e ) {
			...
			const ofertaInput = lastSelectedOferta === 0 ? '' : String(lastSelectedOferta)
